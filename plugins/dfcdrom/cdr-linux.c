...
static void *CdrThread(void *arg);
...
static unsigned char* GetBNormal() {
	return cdbuffer;
}

// threaded reading (with cache)
static long ReadThreaded() {
...
static unsigned char* GetBThreaded() {
	PRINTF("threadc %d\n", found);
...
static long CDRplay(unsigned char *sector) {
